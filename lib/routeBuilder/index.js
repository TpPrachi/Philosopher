...
var _getOne = function(resource, options) {
  var deffered = q.defer();
  db[resource].find({}).toArray(function(err, getAll) {
    getAll.forEach(function(data, err) {
      if (data._id == options) {
        deffered.resolve(data);
      }
      if(err) {
...
var _patch = function(resource, options, values) {
  var deffered = q.defer();
  // db.users.update({_id  : db.ObjectID(options)}, {$set: values}).toArray(function(err, data) {
  //   if(err) {
  //     deffered.reject(err);
  //   }
  //   console.log(data);
