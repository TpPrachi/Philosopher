...
(function() {
  ...
  router.post('/signup', function(req, res, next) {
    var user = req.body;
    db['users'].findOne({$or : [{email : user.email}, {username : user.username}]}, function(err, getUser) {
      if (!getUser) {
          ...
          bcrypt.genSalt(10, function (err, salt) {
            ...
            bcrypt.hash(user.password, salt, function (err, hash) {
              ...
              db['users'].insert(user, function(err, data) {
                ...
                db['usersmapped'].insert({userId:data.insertedIds[0], username:user.username, email:user.email, communityCount: 0,fullname: "",biolosophy:"",location:"",profilePhoto:""});
        ...
        }else if (getUser.email == user.email) {
  ...
  // UserId kevi rite send karse aeee question 6 forgot password ma? - Prachi
