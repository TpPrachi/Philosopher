              ...
              db['users'].insert(user, function(err, data) {
                ...
                mailer.send('sign-up', user, user.email, function sendMailCallback(e, b) {
                  if (e) {
                    ...
                    logger.info(user.email + ' got registered sucessfully');
                  }
                });
                delete user.password;
                delete user.tempPassword;
                res.status(200).send({"sucess":true, "message":"User created successfully.", user: user});
              });
            });
          });
        } else {
                        ...
                        if (isMatch && !err) {
                          ...
                          mailer.send('login', user, user.email, function sendMailCallback(e, b) {
                            if (e) {
                              ...
                              logger.info(user.email.underline + ' got registered sucessfully');
                            }
                          });
