...
(function() {
  ...
  router.post('/signup', function(req, res, next) {
    ...
    if(!_.isUndefined(user) && !_.isUndefined(user.password) && user.password != '') {
        ...
        bcrypt.genSalt(10, function (err, salt) {
            ...
            bcrypt.hash(user.password, salt, function (err, hash) {
                ...
                db['users'].insert(user, function(err, data) {
                  if(err) {
                    logger.error(err);
                    res.status(501).send({"sucess":false, "message":err});
                  }

                  // maaped with user table for later use.
                  db['usersmapped'].insert({fullName:user.firstname + ' ' + user.lastname, id:data.insertedIds[0]}, function(err, data) {
                    if(err) {
                      logger.error(err);
              ...
              bcrypt.hash(req.body.password, salt, function (err, hash) {
                  ...
                  comparePassword(req.body.password, user.password, function (err, isMatch) {
                    if (isMatch && !err) {
                      ...
                      inputForToken['username'] = user.username;
                      inputForToken['password'] = user.password;
                      inputForToken['timestamp'] = new Date().getTime();

                      var token = jwt.encode(user, config.secret);
                      var UUID = uuidV1();
                      db['tokenmapped'].insert({token:'JWT ' + token,uuid:UUID,createDate:new Date()}, function(err, data){
                        console.log("Token Saved to Token Mapped");
                      });
                      // return the information including token as JSON
                      //res.json({success: true, token: 'JWT ' + token});
                      res.status(200).json({success: true, {token: UUID,username:user.fullname, userId:user._id}});
                    } else {
