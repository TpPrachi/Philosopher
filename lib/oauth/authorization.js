  ...
  router.post('/login', function(req, res, next) {
    if(!_.isUndefined(req.body.email) && !_.isUndefined(req.body.password)) {
      db['users'].findOne({
        ...
        tempPassword: req.body.password // We are not check password directly this way,
                      // We need to compare hash generate from passwprd which provided in request
                      // We are compare that with store password in user table
                      // Yes, there is problem but you need to find it by tracing code.
                      // mane find kari ne ko ne su problem 6...!!! - Jaydip
      }, function(err, user) {
        if (err) {
          logger.error(err);
