        ...
        bcrypt.genSalt(10, function (err, salt) {
          ...
          bcrypt.hash(user.password, salt, function (err, hash) {
            ...
            db['users'].findOneAndUpdate({_id: db.ObjectID(req.params.userId),email: req.params.email}, {$set: user}, function(err, updatedUser) {
              // tokenmapped table ma pan update karva ni jarur nathi; - Jaydip
              // Actual ma user forgot karse to data send karsu mail per okkkk...
              // pa6i user aee creditial thi login to karse tyanre automatic mapped thase ne aetle kav 6u
              db['tokenmapped'].findOneAndUpdate({userId : db.ObjectID(user._id)},{$set : {token:'JWT ' + token, userId:user._id, uuid:UUID, modifiedDate:new Date()}}, function(err, data){
                ...
                // I think no need to return user data for forgot passwod request - Jaydip
